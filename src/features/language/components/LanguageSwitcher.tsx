  ...
  return (
    <div suppressHydrationWarning className="relative inline-block text-left" ref={containerRef}>
      <button
        ...
        disabled={isPending}
        type="button"
        className="flex items-center gap-2 hover:text-brand-yellow transition font-medium text-slate-200 cursor-pointer bg-white/5 hover:bg-white/10 px-2.5 py-1 rounded-sm border border-white/10 text-[11px]"
      >
      ...
      {isOpen && (
        <div className="absolute right-0 mt-1.5 w-36 bg-white border border-slate-100/60 rounded-sm z-50 py-1 animate-in fade-in slide-in-from-top-1 duration-150">
          {languages.map((lang) => {
